...
.hdr{position:sticky; top:0; background:rgba(255,255,255,0.85); backdrop-filter:blur(12px); z-index:10; border-bottom:1px solid var(--x-border); display:flex; align-items:center; padding:10px 16px; gap:20px;}
...
.u-row{display:flex; align-items:center; padding:12px 16px; border-bottom:1px solid var(--x-border); transition:0.2s; cursor:pointer;}
...
<div class="app">
    <div class="main">
        ...
        <?php if(empty($users)): ?>
            <div class="empty-state">این کاربر هنوز کسی را دنبال نکرده است.</div>
        <?php endif; ?>
        ...
        ?>
        <div class="u-row" onclick="location.href='profile.php?id=<?=$u['id']?>'">
...
function toggleFollow(e, userId, btn) {
    ...
    // فرض بر این است که actions.php?action=follow وضعیت فالو را تغییر داده و نتیجه را برمی‌گرداند
    let fd = new FormData();
